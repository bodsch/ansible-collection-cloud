...
class NextcloudClient(Occ):
    ...
    def run(self):
        """
        """
        # self.occ = Occ(module=self.module, owner=self.owner, working_dir=self.working_dir)
        ...
        if self.command == "maintenance:install":
            ...
            if rc == 0 and not installed:
                ...
                install_cfg = dict(
                    ...
                    database=dict(
                        type=self.database.get("type", None),
                        hostname=self.database.get("hostname", None),
                        port=self.database.get("port", None),
                        schema=self.database.get("schema", None),
                        username=self.database.get("username", None),
                        password=self.database.get("password", None),
                    ), admin=dict(
                        username=self.admin.get("username", None),
                        password=self.admin.get("password", None),
                    )
                )
        ...
        if self.command.startswith("background"):
            ...
            if crontype in ["ajax", "cron", "webcron"]:
                return self.background_job(crontype)
